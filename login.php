...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($result->num_rows === 1){
        ...
        if(password_verify($password, $user['password'])){
            ...
            // Tandai role admin jika email tertentu
            if($user['email'] === 'jmacleaningservice@hotmail.com'){
            ...
            $error = 'Wrong password. <a href="forgot_password.php" class="font-bold hover:underline">Reset Password</a>';
        }
    } else {
...
if (isset($_SESSION['message'])) {
    echo "<p class='text-center text-green-600'>{$_SESSION['message']}</p>";
    unset($_SESSION['message']);
}

if (isset($error)) {
    echo "<p class='text-center text-red-600'>{$error}</p>";
}
?>
    ...
    <div class="hidden lg:block w-1/2 bg-cover bg-center rounded-l-4xl">
      <div>
        ...
        <p class="font-normal text-center mb-6 text-cyan-900">
          Click below if you need to create an account.
        </p>
        <div class="flex justify-center">
          <a href="signup.php" class="block text-center w-1/2 border-2 border-cyan-900 text-cyan-900 p-3 rounded-full hover:bg-cyan-900 hover:text-white transition">
            Sign Up
          </a>
        </div>
      </div>  
